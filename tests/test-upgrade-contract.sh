...
expected_llvm=060be17654102019e14810c3f948ef85a490755f
...
[[ "$(git -C "${repo_root}/llvm" config --get remote.origin.url)" == *"bloomberg/clang-p2996"* ]] ||
    fail "llvm submodule origin remote is not bloomberg/clang-p2996"
[[ -z "$(git -C "${repo_root}/llvm" status --porcelain)" ]] ||
    fail "llvm submodule is not clean"

grep -qx 'LLVM_VERSION="21.0.0"' "${make_script}" ||
    fail "LLVM_VERSION is not 21.0.0"
grep -qx 'LLVM_SOURCE_REVISION="060be17654102019e14810c3f948ef85a490755f"' "${make_script}" ||
...
grep -q '060be17654102019e14810c3f948ef85a490755f' "${patch_readme}" ||
    fail "Bloomberg p2996 commit provenance is missing"
grep -q 'LLVM 21.0.0' "${patch_readme}" ||
    fail "patch documentation does not name LLVM 21.0.0"
...
grep -q -- '--clobber' "${workflow}" ||
    fail "release cannot replace existing assets"
bash "${repo_root}/tests/test-release.sh"
